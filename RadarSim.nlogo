...
patches-own [energy source]
antennas-own [mode next-pulse last-pulse]
...
to go
  ;if ticks = 1 [ask antennas[transmit]]
  ask antennas[radar-cycle]
  ask targets[reflect]
  ask patches[propagate]
...
to transmit
  ...
  set next-pulse (ticks + pulse-interval)
  set last-pulse ticks
  show (word "Current tick: " ticks ", Next pulse: " next-pulse)
  receive
end
...
to propagate
  if (energy > 0.0) [
    let source-dist distance source

    ;signal decay as it travels through the air
    ;this should be the main factor in the signal's decay
    if (any? neighbors4 with [energy < [energy] of myself]) [
      ask (neighbors4 with [energy < [energy] of myself]) [
        if (distance [source] of myself) > source-dist [
          ;show (word "source-dist " source-dist ", distance of neighbor " distance [source] of myself)
          set energy ([energy] of myself) * propagation-strength
          set pcolor energy * 9.9
          set source [source] of myself
        ]
      ]
    ]

    ;signal decay as time passes
...
305
87
Should be very low to simulate EM wave near lightspeed
9
0.0
1
...
292
116
Roughly represents signal power
9
0.0
1
